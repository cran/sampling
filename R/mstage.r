...
if(!missing(stage)) 
  {number=length(stage)
   for(i in 1:length(stage))
    if(!(stage[i] %in% c("stratified","cluster",""))) stop("the stage argument is wrong")
  }
else number=length(size)
if(number>1) 
	{if(!missing(varnames))
	 {  
       if(!is.list(size)) stop("the size must be a list")
       size=as.list(size)
	...
	else {size1=size[[1]];varnames1=NULL
           if(method %in% c("systematic","poisson")) pik1=pik[[1]]}
      }
else {size1=size
      if(method %in% c("systematic","poisson")) pik1=pik}
if(description)
 cat("STAGE 1","\n")
if(missing(stage))
 {if(missing(varnames))
 ...
 if(stage[1]=="stratified") 
  {s=strata(data,varnames1,size1, method, pik=pik1, description)
   dimension_st=table(s$Stratum)
       ...
       for(i in 1:nl)
         dimension_cl[i]=nrow(subset(s,as.factor(s[,m])==lev[i]))
       } 
...
for(ii in 1:k)
                {
                 r=res[[j-1]][(limit+1):(limit+dimension_st[ii]),]
                 r=getdata(data,r) 
...
for(l in 1:nlevels(as.factor(s3[,m])))
	dimension=c(dimension,table(s3[,m])[l])
s1=rbind(s1,s3)
}
}
}
else
 if(stage[[j-1]]=="cluster") 
{
...
for(ii in 1:k)
                {
                 r=res[[j-1]][(limit+1):(limit+dimension_cl[ii]),]
                 r=getdata(data,r) 
                 m=match(varnames[[j]],names(r))
                 if(method %in% c("systematic","poisson")) 
                 { 
			...
			mm=match(r[1,m1],levels(factor(data[,m2])))
                  pikk=as.numeric(pik[[j]][[mm]])  
                  if(!is.null(r)) 
		      s3=cluster(r,clustername=varnames[[j]],size=size1[[ii]],method=method, pik=pikk,description)                        
                  else s3=NULL
                  }
                 else s3=cluster(r,clustername=varnames[[j]],size=size1[ii],method=method, pik,description)                        
...
s1=rbind(s1,s3)
		  ...
		  if(j>1)
               {k=length(dimension) 
                ...
                if(k>=1)
                for(i in 1:k)
                 for(ii in 1:length(dimension[[i]]))
                {
                 r=res[[j-1]][(limit+1):(limit+dimension[[i]][ii]),]
                 count=count+1
                 if(method %in% c("systematic","poisson")) 
                 {index=res[[j-1]][(limit+1):(limit+dimension[[i]][ii]),]$ID_unit 
                 pikk=pik[[j]][index]
                 if(!is.null(r))
                 s2=strata(r,NULL,size=size1[count],method=method, pik=pikk,description)                        
                 else s2=NULL
                 }
                else s2=strata(r,NULL,size=size1[count],method=method, pik,description)                        
                limit=limit+dimension[[i]][ii]              
                if(method=="srswr")
                {s2=cbind.data.frame(r[s2$ID_unit,]$ID_unit,s2$Replicates,s2$Prob,r[s2$ID_unit,]$Prob*s2$Prob)
                 colnames(s2)=c("ID_unit","Replicates",paste("Prob_",j,"_stage"),"Prob")
                 }
 		  else if(!is.null(s2))
		  {
		   s2=cbind.data.frame(r[s2$ID_unit,]$ID_unit,s2$Prob,r[s2$ID_unit,]$Prob*s2$Prob)
		   colnames(s2)=c("ID_unit",paste("Prob_",j,"_stage"),"Prob")
...
s1=rbind(s1,s2)
 ...
 if(missing(stage))
 {if(missing(method))
 	s1=strata(result, stratanames=NULL, size=size[[j]], description=description)
  else  
if(method=="poisson" | method=="systematic") 
   s1=strata(result,stratanames=NULL, size=size[[j]], method=method, pik=pik[[j]], description=description)
else 
  s1=strata(result,stratanames=NULL,size=size[[j]], method=method, description=description)
  if(method=="srswr")
  {s1=cbind.data.frame(result[s1$ID_unit,]$ID_unit,s1$Replicates,s1$Prob,result[s1$ID_unit,]$Prob*s1$Prob)
   colnames(s1)=c("ID_unit","Replicates",paste("Prob_",j,"_stage"),"Prob")
  }
  else
  {
   s1=cbind.data.frame(result[s1$ID_unit,]$ID_unit,s1$Prob,result[s1$ID_unit,]$Prob*s1$Prob)
   colnames(s1)=c("ID_unit",paste("Prob_",j,"_stage"),"Prob")
   }
  }
}
if(!is.null(s1)) 
