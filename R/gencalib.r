gencalib<- function(Xs, Zs, d , total, q=rep(1, length(d)), method=c("linear", "raking", "truncated", "logit"), bounds=c(low=0,upp=10),description=FALSE,max_iter=500)
{
if (any(is.na(Xs)) | any(is.na(Zs)) | any(is.na(d)) | any(is.na(total)) | any(is.na(q)))
...
if(is.vector(Xs) & length(total)!=1)
	stop("Xs and total have different dimensions") 
if(!(is.matrix(Zs) | is.array(Zs))) Zs=as.matrix(Zs)
if(any(dim(Xs)!= dim(Zs))) stop("Xs and Zs have different dimensions")
...
lambda1 = ginv(t(Xs * d *q) %*% Zs, tol = EPS) %*% (total-c(t(d) %*% Xs))
...
if(method=="truncated")
     ...
     if(missing(bounds)) stop("Specify the bounds")
     g=1+q*c(Zs%*%lambda1)
     list=1:length(g)
     l=0
     g1=g
     Xs1=Xs; Zs1=Zs; d1=d; t2=total;list1=1:length(g);q1=q
     while(l<max_iter)  
        { l=l+1
          if(any(g<bounds[1]) | any(g>bounds[2]))  
		    ...
		    g[g>bounds[2]]=bounds[2]
             ...
             t1=c(t(d1)%*%Xs1) 
             lambda1 = ginv(t(Xs1 * d1*q1) %*% Zs1, tol = EPS) %*% (t2-t1)    
     	          if(length(list1)>1)
...
for(l in 1:max_iter)
	...
	lambda=lambda-ginv(phiprim,tol=EPS)%*%phi
     	w1=as.vector(d*exp(Zs%*%lambda*q))
      if(any(is.na(w1))) {warning("No convergence");g=NULL;break}
      tr=crossprod(Xs,w1)
      if(max(abs(tr-total)/total)<EPS1) break; 
        }
   if(l==max_iter) {warning("No convergence");g=NULL;}
   else g=w1/d
}
else if(method=="logit")
{
if(bounds[2]<=1 || bounds[1]>=1 || bounds[1]>bounds[2]) stop("The conditions low<1<upp are not fulfilled")
if(bounds[1]>bounds[2]) stop("The conditions low<upp are not fulfilled")
...
phiprime=T%*%Zs1
lambda1=lambda1-ginv(phiprime,tol=EPS)%*%(as.vector(phi)-t2)
u=exp(A*(Zs1%*%lambda1*q1))
F=g1=(bounds[1]*(bounds[2]-1)+bounds[2]*(1-bounds[1])*u)/(bounds[2]-1+(1-bounds[1])*u)  
if(any(is.na(g1))) {warning("no convergence");g1=g=NULL;break}
g[list1]=g1
tr=crossprod(Xs,g*d)
if(max(abs(tr-total)/total)<EPS1 & all(g>=bounds[1] & g<=bounds[2])) break; 
}
